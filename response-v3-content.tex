...
\bigskip
  ...
  \REVIEW{DONE}{Submit the point-by-point response to the
...
\REVIEW{DONE}{Acronyms need to be defined at their first occurrences
  (e.g., AI in the intro). A table of acronyms is included in table 1,
...
\REVIEW{DONE}{Figure 2: please present the equations as equations. The
...
\REVIEW{DONE}{Regenerate the figures into pdf format (the current
  ...
  {All images have been included as PDF images}


\REVIEW{DONE}{Table 2: consider reformatting it; it would not fit the
  page width in print.}
...
\REVIEW{DONE}{Figure 5-6: can be combined to one figure with two
...
\REVIEW{DONE}{Figure 7-8: can be one figure with two panels; pop
...
\REVIEW{DONE}{Figure 9-14: combine into one figure.}
  {
...
\REVIEW{DONE}{Reproducibility: submit data/code/README in a
...
\REVIEW{DONE}{Page 6, line 8, what level is “local level”?
}
  {
  ...
  (c) In Section 4, the sentence containing \linelabel{comment:level}  ``local level'' has been modified as follows:
...
\REVIEW{DONE}{ Page 7, Table 1, is “CHD” also a percent? If yes
...
\REVIEW{DONE}{What do the two panels in Figure 4 represent? Please give
  clearer description either in the text or figure caption.
}
  { Better descriptions have been added. }
...
\REVIEW{DONE}{Section 5.2, the ``order for cases sorted by minimum model
...
\REVIEW{DONE}{In Table 3, what is the unit for the error terms? The
...
\REVIEW{DONE}{Please adjust the location of your figures so they do not
...
\REVIEW{DONE}{If I am reading it correctly, Figures 9 and 11 are only
...
\REVIEW{DONE}{The last paragraph of Section 5.2 suits the
  conclusion/discussion for future work section, not in the middle of
  a paper.
}
  { This paragraph has been moved to the conclusion.}


\REVIEW{DONE}{Section 5.3, the authors mentioned two way of constructing
  ...
  { Based on the reviewers suggestion we removed section 5.3 so we could focus on the previous data analysis only. }


\REVIEW{DONE}{The description “We renormalized, so the two basic
...
\REVIEW{DONE}{Paragraph 2 on page 14, “Selu, Relu and Tanh activations
...
\REVIEW{DONE}{How to interpret the “significant positive correlation” in
  ...
  { Based on the suggestion of the reviewer we deleted Section 6}
