...
\REVIEW{DONE}{Please adjust the location of your figures so they do not
  ...
  {The location have een adjusted, but LaTeX tries to optimize the
  placement. We have used the !h flag to force placement. If this
  placement is still not sufficient please let us know. We also
  removed images in order to facilitatie better placement. The
  document is submitted in drafts mode which also could influence the
  placement. We have tried the document in final mode with the LaTeX
  style and it seems the placement is now fixed as we removed images.}
...
\REVIEW{DONE}{If I am reading it correctly, Figures 9 and 11 are only
  ...
  {We have provided in an earlier comment that we now only include the
  Top 10 instead of Top 1, and Top 5 to fulfull the desire of the
  reviewers to have these images combined. However we still separated
  the deaths from teh cases view to ease the readers ability to see
  more details. HOwever we can easily replace the image with the one
  provided earlier if the reviewers desire. This is a small change and
  will appreciate the revieers feedback on this further.}
